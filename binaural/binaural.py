...
class BinauralEngine(QBuffer):
    def __init__(self, parent=None):
        ...
        # Boş bir byte array ile buffer'ı aç
        self.setBuffer(QByteArray())
    ...
    def readData(self, maxlen):
        # QAudioOutput sürekli ses verisi talep ettikçe burası tetiklenir
        data = bytearray()
        num_samples = maxlen // 4  # 1 sample = 2 kanal * 2 byte (16-bit PCM)
        
        for _ in range(num_samples):
            # Sol kanal sinüs hesabı
            val_left = math.sin(self.phase_left) * self.volume * 32767
            ...
            val_right = math.sin(self.phase_right) * self.volume * 32767
            self.phase_right += 2 * math.pi * self.freq_right / self.sample_rate
            if self.phase_right > 2 * math.pi:
                self.phase_right -= 2 * math.pi

            # 16-bit Signed Integer (h) olarak paketle
            try:
                data.extend(struct.pack('<hh', int(val_left), int(val_right)))
            except Exception:
...
class MainWindow(QMainWindow):
    ...
    def update_frequencies(self):
        ...
        # Farkı hesapla ve dalga türünü yaz
        diff = abs(fl - fr)
        wave_type = ""
    ...
    def show_about_dialog(self):
        ...
        about_details = (
            f"{self.lang.get('about_version')}: 1.0.0\n"
